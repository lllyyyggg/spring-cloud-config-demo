package com.lanyage.zuul.userconsumer.config;
...
public class ZipkinConfig {
    ...
    public SpanCollector spanCollector() {  //span（一次请求信息或者一次链路调用）信息收集器
    ...
    @Bean(name = "brave")
    public Brave brave(@Qualifier("spanCollector") SpanCollector spanCollector) {   //作为各调用链路，只需要负责将指定格式的数据发送给zipkin
        logger.info("==> {} has been initialized. <==", "brave");
        Brave.Builder builder = new Brave.Builder("USER-CONSUMER");  //指定service-name
    ...
    @Bean
    public BraveServletFilter braveServletFilter(Brave brave) { //设置server的过滤器，服务端收到请求和服务端完成处理，并将结果发送给客户端。
    ...
    @Bean
    public OkHttpClient okHttpClient(@Qualifier("brave") Brave brave) { //设置client的拦截器。
